  ...
  <form @submit.prevent="handleSubmit">
    <div class="flex flex-col w-screen md:w-96 gap-6 pt-10">
      <div class="flex flex-col gap-2">
        ...
        <InputText v-model="email" id="email" aria-describedby="email-help" class="focus:border-[#3525CD]" />
      </div>

      <div class="flex flex-col gap-2 w-96">
        <label for="password" class="font-semibold">Mot de passe</label>
        <Password v-model="password" toggleMask class="w-96 focus:border-0" fluid :feedback="false" />
      </div>

      <Button label="Se connecter" class="bg-[#4F46E5]" type="submit" :disabled="loginStore.loading"/>
